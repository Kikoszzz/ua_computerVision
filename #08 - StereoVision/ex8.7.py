...
def main():
    ap = argparse.ArgumentParser(description='3D reconstruction from stereo disparity')
    ...
    ap.add_argument('--output', '-o', default='points3d.npz', help='Output npz file for 3D coordinates')
    args = ap.parse_args()
    ...
    # disp from StereoBM is fixed-point (scaled by 16). Convert to disparity in pixels.
    disp = stereo.compute(grayL, grayR).astype(np.float32) / 16.0

    # 8.7 requirement: compute 3D coordinates from disparity map and Q.
    points3d = cv2.reprojectImageTo3D(disp, Q)

    np.savez(args.output, points3d=points3d)
    print('Saved 3D coordinates to:', args.output)
    print('points3d shape:', points3d.shape)
    h, w = points3d.shape[:2]
    print('Sample XYZ at image center:', points3d[h // 2, w // 2])

    # Visualize disparity and depth so 8.6 and 8.7 outputs are clearly different.
    disp_vis = cv2.normalize(disp, None, 0, 255, cv2.NORM_MINMAX)
    disp_vis = np.uint8(disp_vis)

    # Visualize depth from the Z channel of the 3D points.
    depth_z = np.nan_to_num(points3d[:, :, 2], nan=0.0, posinf=0.0, neginf=0.0)
    depth_vis = cv2.normalize(depth_z, None, 0, 255, cv2.NORM_MINMAX)
    depth_vis = np.uint8(depth_vis)

    cv2.imshow('Left rectified', remap_imgl)
    cv2.imshow('Disparity (disp)', disp_vis)
    cv2.imshow('Depth (Z)', depth_vis)
